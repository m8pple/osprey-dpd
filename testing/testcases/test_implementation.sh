...
function usage()
{
    >&2 echo "test_implementation.sh path_to_dpd dmpci_file? --working-dir=dir"
    >&2 echo ""
    ...
    >&2 echo ""
    >&2 echo "   --working-dir : Where to put working files and outputs."
...
>&2 echo "Running ${replicate_count} replicates, starting from start_seed=$start_seed"
../tools/run_replicates.py $dpd_executable $dmpci_file ${replicate_count} $working_dir $start_seed

>&2 echo "Extracting $dmpci_file"
../tools/extract_dmpcas_distributions.py $dmpci_file $working_dir $working_dir $start_seed

>&2 echo "Generating graphs"
../tools/convert_dmpcas_distributions_to_graphs.py $working_dir/$dmpci_name-$dmpci_hash.h5

>&2 echo "Testing"
../tools/evaluate_series.py $working_dir/$dmpci_name-$dmpci_hash.h5 $dmpci_file | tee $working_dir/${dmpci_name}-${dmpci_hash}.csv
